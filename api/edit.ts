...
- If the user provides a reference image, interpret the design and implement it
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (action === 'new-page') {
      let userPrompt = `The user wants to create a new page for this website.\n\nThe user wants: ${message}`;
      if (referenceUrl) {
      ...
      userPrompt += `\n\nGenerate a complete page file. Use the same patterns as other pages in the project. Include data-section attributes on each section.`;
    ...
    if (isGlobal) {
      userPrompt += `\n\nIMPORTANT: This is a GLOBAL section (navigation or footer) that appears on every page. Changes here will affect all pages site-wide. Ensure data-global="true" is preserved.`;
    }
